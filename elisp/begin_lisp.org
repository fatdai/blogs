...
#+BEGIN_SRC elisp
 (message "Hello World!")
#+END_SRC
在语句最后使用 C-j 或者 C-x C-e 进行执行
注意:需要切换到lisp-interaction-mode下进行执行
** C-j 和 C-x C-e 区别
+ C-x C-e 是一个全局的按键绑定,几乎所有地方都能使用,它会将运行的返回值
  显示在minibuffer里.这里需要强调一个概念是返回值和作用是不同的。比如
  前面 message 函数它的作用是在 Minibuffer 里显示一个字符串，但是它的
  返回值是“hello world”字符串。
** 15分钟入门 
内容来自于:http://hahack.com/wiki/elisp-intro.html
#+BEGIN_SRC elisp
